...
def generate_cluster_points(centre_lat, centre_lon, n, base_speed, label, start_ts):
    ...
    for i in range(n):
        ...
        lat = centre_lat + random.gauss(0, 0.005)
        lon = centre_lon + random.gauss(0, 0.005)

        # Temporal progression
        timestamp = start_ts + i * random.uniform(5, 30)

        # Speed variation around base speed
        speed = max(1.0, min(120.0, base_speed + random.gauss(0, 5)))

        # Density inversely related to speed
        density = max(1.0, min(150.0, (150 / max(speed, 1)) * random.uniform(0.8, 1.2)))

        # Flow proportional to speed × density
        flow = max(0.1, min(60.0, (speed * density / 60) * random.uniform(0.8, 1.2)))
...
def main(n_points=5000, n_clusters=6, output="data/gps_data.csv"):
    ...
    total_cluster_points = int(n_points * 0.8) // n_clusters
    noise_count = n_points - total_cluster_points  # ensures exact total
    
    base_pts = total_cluster_points // n_clusters
    remainder = total_cluster_points % n_clusters
